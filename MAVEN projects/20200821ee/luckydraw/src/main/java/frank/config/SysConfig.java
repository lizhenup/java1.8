...
public class SysConfig implements WebMvcConfigurer, InitializingBean{
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        registry.addInterceptor(new LoginInterceptor(objectMapper))
                .addPathPatterns("/api/**")// *表示一级路径匹配 **多级任意匹配
                .excludePathPatterns("/api/user/register")
                .excludePathPatterns("/api/user/login")
                .excludePathPatterns("/api/user/logout");
        //再添加一个判断前端资源请求的拦截器，非登录页面(/*. 排除/index.html)如果没有登录，重定向到/index.html

    }

    public void addInterceptorsI(InterceptorRegistry registry) {
        registry.addInterceptor(new LoginInterceptor(objectMapper))
                .addPathPatterns("/api/index.html")// *表示一级路径匹配 **多级任意匹配
